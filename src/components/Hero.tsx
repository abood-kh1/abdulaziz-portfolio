    ...
    <section aria-labelledby="hero-title" className="relative overflow-hidden pt-16">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-12 px-5 pt-14 pb-16 md:grid-cols-[1.15fr_0.85fr] md:items-center md:px-8 md:pt-24 md:pb-24">
        <div>
          <div className="flex items-center gap-5">
            ...
            <div>
              <p className="rule-label text-accent">{t.hero.badge}</p>
              <h1 id="hero-title" className="mt-3 font-display text-[clamp(2.6rem,6.5vw,4.8rem)] leading-[1.02] font-medium tracking-tight text-cream">
                <span className="rise-line"><span>{t.hero.titleFirst}</span></span>
                <span className="rise-line"><span>{t.hero.titleLast}</span></span>
              </h1>
            </div>
          </div>
          <p className="mt-5 max-w-xl border-l-2 border-accent ps-5 text-[clamp(1.02rem,2vw,1.25rem)] leading-relaxed text-sand">
            {t.hero.tagline}
          </p>
          <div className="mt-8 flex flex-wrap items-center gap-4">
            ...
            >
              {t.hero.viewProjects} <span className="inline-block transition-transform group-hover:translate-x-1">↓</span>
            </Link>
            <a
              href="#contact"
          ...
          <dl className="mt-6 flex flex-wrap gap-x-8 gap-y-3 font-mono text-[11px] tracking-[0.16em] text-dim uppercase">
            <div className="flex gap-2"><dt className="text-fog">{t.hero.focus}</dt><dd>{t.hero.focusVal}</dd></div>
            <div className="flex gap-2"><dt className="text-fog">{t.hero.data}</dt><dd>{t.hero.dataVal}</dd></div>
            <div className="flex gap-2"><dt className="text-fog">{t.hero.edge}</dt><dd>{t.hero.edgeVal}</dd></div>
          </dl>
        </div>
        ...
        >
          <div className="flex items-center justify-between font-mono text-[10px] tracking-[0.2em] text-dim uppercase">
            <span>{t.hero.requestLifecycle}</span>
          ...
          <div className="relative mt-4">
            <div aria-hidden="true" className="absolute top-2 bottom-2 start-[7px] w-px bg-line" />
            <div aria-hidden="true" className="packet-dot absolute start-[4.5px] h-[7px] w-[7px] rounded-full bg-accent" />
              ...
              {layers.map((l, i) => {
                ...
                return (
                  <li
                    ...
                    className={`relative ms-7 rounded-md border px-4 py-3 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-accent ${isActive ? "border-accent/60 bg-panel" : i === 1 || i === 2 ? "border-accent/30 bg-panel" : "border-line bg-ink"} ${isActive ? "scale-[1.01]" : ""}`}
                  >
                    <div className="flex items-baseline justify-between gap-3">
                      <span className={`font-mono text-sm font-semibold tracking-[0.2em] ${isActive ? "text-accent" : i===1||i===2 ? "text-cream" : "text-fog"}`}>{l.tag}</span>
                    ...
                    <span className={`mt-1.5 block font-mono text-[10.5px] ${isActive ? "text-cream" : i === 1 ? "text-accent" : "text-dim"}`}>
                      {isActive ? l.hint : i === 1 ? <><span className="caret">▌</span> {t.hero.layersDetailAuth}</> : ""}
                    </span>
                  </li>
                );
              })}
          ...
          <p className="mt-4 border-t border-line pt-3 font-mono text-[10.5px] leading-relaxed tracking-wide text-dim">
            <span className="text-fog">{t.hero.systemsBehind}</span> {t.hero.systemsDesc} <span className="hidden md:inline">{t.hero.hoverHint}</span>
          </p>
        </div>
      </div>
      ...
      <div className="relative border-y border-line bg-[#0c0b09]">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-2 overflow-x-auto px-5 py-3 font-mono text-[11px] tracking-[0.22em] whitespace-nowrap text-dim uppercase md:px-8">
          ...
          <span className="hidden text-dim/70 sm:inline">{t.hero.strip.desktop}</span>
        </div>
      </div>
    </section>
  );
