...
export async function askAssistant(
  ...
  // Try Gemini if key exists and looks like Gemini key (AIza...)
  const hasValidKey = apiKey && apiKey.startsWith("AIza") && apiKey.length > 20;
  if (hasValidKey) {
    try {
  ...
  // If not found good match, localRetrieve already returns a helpful not-found message
  // Don't prepend "local mode" banner when Gemini key is invalid — just return retrieved
  if (hasValidKey === false && apiKey) {
    // key exists but invalid format (like AQ...), explain
    const hint = isArEffective
      ? " (ملاحظة: مفتاح Gemini الحالي بصيغة غير صحيحة — يجب أن يبدأ بـ AIza... من Google AI Studio. أجيب الآن من المعرفة المحلية)\n\n"
      : " (note: current Gemini key format is invalid — should start with AIza... from Google AI Studio. Answering from local knowledge)\n\n";
    return hint + retrieved;
  }
  return retrieved;
}
